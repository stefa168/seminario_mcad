...
\title[Seminario 8]{Impossibilita del consenso nel modello a rete asincrono, una soluzione randomizzata}
\subtitle{Seminario 8}
\author[Liberatore, Mulone, Palazzo, Porta]{Daniele Liberatore, Matteo Palazzo, \\ Stefano Porta, Alberto Mulone}
...
\vbox{%
    \begin{itemize}
        ...
        \item L'operazione $send(m)_{i, j}$ (i.e. invio del messaggio $m$ dal processo $i$ al processo $j$) viene simulata aggiungendo il messaggio $m$ al fondo della coda relativa alla variabile $x(i, j)$.
        \item Per quanto riguarda la ricezione di messaggi, invece, il generico processo $i$, ciclicamente verifica la presenza di eventuali nuovi messaggi da parte di un qualche processo $j$ posti nella coda relativa alla variabile condivisa $x(j, i)$.
        \item Il modo in cui il singolo processo elabora un certo messaggio $m$ rimane invariato.
    \end{itemize}
    }
}
\end{frame}
